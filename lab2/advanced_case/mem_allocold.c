...
{
    for (size_t i = 0; i < NB_MEM_POOLS - 1; i++) {
      memory_leak_check_fast_pool(mem_pools[i]);
    }
    // memory_leak_check_standard_pool();
...
void memory_leak_check_fast_pool(mem_pool_t mem){
  size_t pool_size = mem.pool_size;
  int flag_arr[pool_size];
  for (int i = 0; i < pool_size; i++)
  {
  ...
  {
    if(flag_arr[i] == 1){
      printf("There is a leak in fast pool %d\n", mem.pool_id);
      break;
    }
  }
  print_state_array(flag_arr, pool_size);
...
void memory_leak_check_standard_pool(mem_pool_t mem) {
  mem_standard_free_block_t *first_free = (mem_standard_free_block_t*) mem.first_free;
  mem_standard_block_header_footer_t *header = &first_free->header;
  if (mem.pool_size != get_block_size(header)) {
    printf("There is a memory leak in standard pool\n");
    print_standard_pool(mem);
  }
}
